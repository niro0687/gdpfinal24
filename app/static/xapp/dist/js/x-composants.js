...
function XAuth ({children, idType, lostPasswordText, lostPasswordURL, titleSize, idLabel, usernameIcon, usernameStyle, passwordStyle, button, center, checkResponse, 
	passwordMinLength, url, usernameBd, passwordBd, fieldWidth, fieldStyle, register, token, registerURL, registerText, dataType, successAuthFunc, requestErrorFunc, lostPassword}) {
	...
	const registerContent = function() {
		...
		return <div className={cl} style={st}>
			{ registerURL == null ? 
				null : 
				<a className="x-t-info x-link x-pointer xfosi13 xfowebo" href={registerURL}>{registerText}</a>
			}
		</div>
	}();
	...
	const getLostPassworgContent = React.useMemo(() => {
		if (lostPassword) {
			return <div className="xwi100per x-child-center xmato0" >
				<a href={ lostPasswordURL } className="xfosi12 xfowebo x-low-emphasis" >{ lostPasswordText }</a>
			</div>
		}
		return null;
	}, []);
...
XAuth.defaultProps = {
	...
	fieldWidth: "380px",
	lostPassword: true,
	lostPasswordText: 'I lost my password',
	lostPasswordURL: '',
	register: true,
	tokens: {},
	dataType: null,
