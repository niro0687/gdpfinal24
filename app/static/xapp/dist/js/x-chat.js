...
function XDisplayUserToCreateConversationComponent ({userSug, getName, onSelectUser, createGroupClick, close}) {
	...
	return <XPopupBox 
		...
		<div onClick={createGroupClick} className="xdigr x-center x-create-conv-group xalitce xmato20 x-pointer">
			<div className="x-child-center x-black-01 x-circle-50">
				<i className="fa fa-users"></i>
			</div>
			<div>
				<p className="xfowebo">Créer une groupe de conversation</p>
			</div>
		</div>
		
		<p className="xmale20 xmato20 xfowebo">Utilisateurs</p>
...
function XStartConvBoxComponent ({url, placeholder, close ,title, token, user, style, getConversationJustCreated}) {
	...
	return <div className="xpore x-start-conv x-shadow" style={style}>
    	<div onClick={close} className="x-square-40 xdifl xjucoce xalitce xpoab close-start-conv x-pointer">
...
function XConversationList ({data, getName, open, createConvClick, createConvURL, removeThisRoom}) {
	...
	const conv_filtered = React.useMemo(() => {
		let all = [];
		
		data.forEach((p) => {
			if (p.type == 'private') {
				let name = p.user.lastName + ' ' + p.user.firstName;
	...
	const [focusConvOption, setFocusConvOption] = React.useState(null);
	...
	const requestDeleteConv = async function () {
		let url = '/delete_conversation/' + focusConvOption.crypted_id + '/';
	...
	return <div className={listClassName}>
		...
		{ data.length == 0  ?
			<p className='x-low-emphasis xfosi13 xfowebo xtealce xmato100'>Vous n'avez aucune conversation jusqu'a maintenant...</p> :
			<div className="xpoab xwi100per" style={convStyle}>
				{ conv_filtered.map((conv) => (
					<XConversation 
						key={conv.crypted_id}
						open={open}
		...
		<div ref={op} className="conv-right-option xpoab x-bo-ra_10 xwi100per x-le_0">
			...
			<div className="each-conv-option x-center xalitce xhe55 xdigr xwi90per x-pointer" onClick={deleteConv} >
				<div className="x-child-center x-square-40" >
...
function XConversationHeader ({close, data, clickSearch, status_text}) {
	...
	return <div className={headerClass}>
		<div className="info">
			<div className="x-pointer x-close-chat hide">
				<i className="fa fa-chevron-left"></i>
			</div>
			<div className="user xmale15">
				<div className="pic">
					<XUserProfilePicture
						width={47}
						name={data.name}
				...
				<div className="data">
					...
					<span className={statusClass}>{status_text}</span>
				</div>
			</div>
		</div>
		<div className="option">
			<div className="x-pointer search each-option" onClick={clickSearch}>
				<i className="fa fa-search"></i>
			</div>
			<div className="x-pointer close-chat each-option show" onClick={close}>
				<i className="fa fa-times"></i>
			</div>
		</div>
	</div>
...
function XTextMessageComponent ({message, onSent, token, sendingUrl, option, projectID}) {
	...
	const getMessageDate = function (message) {
		let d = new Date(message.datetime);
	...
	return <div className={messageClassName} onContextMenu={messageOption} id={message.id} onClick={messageClick}>
		{ message.status != 'sending' && message.showDate ? <p className='xtealce xfosi12 xfowebo xmato20 xmabo15'>{ getMessageDate(message) }</p> : null }
		...
		<div className="x-message-container">
			...
			<div className="x-message-data speech">
				<div className="speech-data">
					<div className={message.status == "sending" ? "x-pointer play-pause" : "x-pointer play-pause can-play"} target={'aud' + message.id}>
						<i className="fa fa-play"></i>
					</div>
					<div className="waves">
...
function XImageMessageComponent ({message, onSent, token, sendingUrl, option, projectID, visual}) {
	...
	const messageClassName = "x-pointer x-message " + position + getMessageStatus;
	...
	return <div className={messageClassName} onContextMenu={messageOption} id={message.id} onClick={messageClick}>
		{ message.status != 'sending' && message.showDate ? 
		<p className='xtealce xfosi12 xfowebo xmato20 xmabo15'>{ getMessageDate(message) }</p> : null }
		<div className="x-message-container">
			{profile}
...
function XOpenConversation ({style, syncConversationList, token, element, parent, projectID, visual, sentSound, close, 
	conversationData, urlSending, convID, header, messages, messageForm, convCryptedID, removeThisRoom}) {
	...
	const [searchMessageValue, setSearchMessageValue] = React.useState('');

	const handleSearchMessageValue = (val) => {
	...
	const filteredMessageSent = React.useMemo(() => {
		let ret = [];

		messageSent.forEach((mess) => {
			if (mess.text) {
		...
		socket.onmessage = function (e) {
			...
			if (Array.isArray(response)) {
				...
				if (response.action == 'new') {
					...
					setMessageSent((prev) => {
						let arc = prev.filter((mess) => mess.id != response.id);
						if (arc.length == 0) {
	...
	const textValueToCopy = React.useRef(null);
	...
	const downloadDOM = React.useRef(null);
	...
	const copyText = (data) => {
		if (focusedMessageOption != null && textValueToCopy != null) {
			...
			navigator.clipboard.writeText(textValueToCopy.current.value).then(() => {
				let message = textValueToCopy.current.value;
				__.xAlert('Message', `Text copié: '${ __.getShortText(message, 25) }'`, 'info');
				closeMessageOption();
			}).catch(err => {
	...
	return <div className={conversationBoxClass}>
		...
		<div ref={messOp} className="x-message-option xpoab">
			...
			<div className="each-conv-option x-center xalitce xhe55 xdigr xwi90per x-pointer" onClick={ () => copyText(focusedMessageOption) }>
				<div className="x-child-center x-square-40">
			...
			{ (focusedMessageOption != null) && ( focusedMessageOption.audio || focusedMessageOption.image || focusedMessageOption.video || focusedMessageOption.file) ?
			<div className="each-conv-option x-center xalitce xhe55 xdigr xwi90per x-pointer" onClick={ () => downloadFile(focusedMessageOption) }>
				<div className="x-child-center x-square-40">
					<i className="fa fa-arrow-down xfosi22"></i>
				</div>
				<div className="">
					<p className="xfosi12 xfowebo">Télécharger le fichier</p>
				</div>
			</div> : null
			}
			...
			{ focusedMessageOption != null && (focusedMessageOption.file || focusedMessageOption.audio || focusedMessageOption.video || focusedMessageOption.image) ?
				<a href={ getFileURL(focusedMessageOption) } className='xop0 xwi0per xhe0per' ref={downloadDOM} download></a> : null
			}
			...
			{ focusedMessageOption != null && focusedMessageOption.text ?
				<input 
					...
					className={'xop0'}
					ref={textValueToCopy}
					defaultValue={focusedMessageOption.text} /> : null
			}
			
		</div>
		
		{ showSearch ? 
		<div className='xpoab xle20 xri20 xto70 x-shadow x-search-message-form xzin4 xbora15'>
			<XSearch
				style={{
					width: '100%'
				}}
		...
		<div className="x-all-messages x-all-messages-display xpoab xwi100per"  style={allMessageStyle}>
			...
			{filteredMessageSent.map((message) => <XMessage 
